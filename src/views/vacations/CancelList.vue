...
const onClickItem = (id) => {
  // console.log(id)
  router.push({ name: '휴가취소상세', params: { id: id } })
}
</script>
                ...
                <table class="table-3 dataTable-table max-w-full w-full">
                  ...
                  <tbody>
                    <tr
                      ...
                      :key="index"
                      @click="onClickItem(item.cancelId)"
                    >
                      <td class="py-3">
                        {{ item.vcReqDto.vcTypeDto.typeName }}
                      </td>
                      <td class="py-3" v-if="item.vcReqDto.reqDays > 1">
                        {{ item.vcReqDto.startDate }} -
                        {{ item.vcReqDto.endDate }}
                      </td>
                      <td class="py-3" v-else>
                        {{ item.vcReqDto.startDate }}
                      </td>
                      <td class="py-3">{{ item.cancelReqDate }}</td>
                      <td class="py-3">
                        <span
                          class="px-3 py-1 rounded-lg bg-primary dark:text-white text-white text-xs"
                          :class="setStatusStyle(item.cancelStatus)"
                          >{{ item.cancelStatus }}</span
                        >
                      </td>
                    </tr>
                  </tbody>
