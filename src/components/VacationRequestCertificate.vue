...
export default {
  ...
  setup(props) {
    ...
    onMounted(() => {
      ...
      const typeId = props.item.vcReqDto.vcTypeDto.typeId
      if (props.item.vcReqDto.reqDays <= 1 && (typeId == 3 || typeId == 2)) {
        alHalfType.value = true
      }

      // Date 객체로 변환
      const start = new Date(props.item.vcReqDto.startDate)
          ...
          <tbody>
            ...
            <tr>
              ...
              <td
                ...
                v-else
              >
                {{ item.vcReqDto.startDate }} - {{ item?.endDate }}
              </td>
            </tr>
            <tr class="h-4">
              <td class="bg-white dark:bg-foreground" colspan="2"></td>
            ...
            <tr>
              ...
              >
                ...
                <p class="mb-2">
                  - 공가(예비군/민방위)는 사전에 통지서를, 사후에 참석증을
                  반드시 제출
                </p>
                <br />
              </td>
